...
class FirSyntheticPropertiesScope private constructor(
    ...
    override fun processPropertiesByName(name: Name, processor: (FirVariableSymbol<*>) -> Unit) {
        ...
        if (!getterFound) {
            val getterName = syntheticNamesProvider.possibleAdditionalGetterNamesByPropertyName(name, containingClassSymbol) ?: return
            baseScope.processFunctionsByName(getterName) {
                checkGetAndCreateSynthetic(name, getterName, it, needCheckForSetter = false, processor)
            }
        }
    }
